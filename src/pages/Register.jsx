...
export default function Register() {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			// Try popup first (works when browser allows popups)
			...
			if (code === 'auth/popup-blocked' || code === 'auth/operation-not-supported-in-this-environment') {
				try {
					// persist chosen role across redirect
					localStorage.setItem('ridit_google_user_type', userType);
					await signInWithRedirect(auth, provider);
					// Note: the redirect will navigate away; result processed in useEffect on mount
					return;
				} catch (redirErr) {
					setError(redirErr.message || 'Redirect sign-in failed');
				}
			} else {
				// Firebase errors don't have axios response; prefer err.message
	...
	// Handle results from redirect sign-in (fallback)
	useEffect(() => {
		let mounted = true;
		const processRedirect = async () => {
			try {
				...
				// No-op; redirect may not have result or user cancelled
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			setTimeout(() => navigate("/login/seller", { state: { registered: true } }), 900);
		} catch (err) {
		...
		<div className="auth-container">
			<div className="auth-card register-card">
				<div className="card-header">
					...
					<h1>Join Ridit</h1>
					<p className="tagline">Be part of the waste revolution</p>
				</div>
